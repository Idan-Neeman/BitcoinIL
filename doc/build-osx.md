...
This guide describes how to build bitcoinild, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build BitcoinIL Core from source.
...
### 4. Clone BitcoinIL repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the BitcoinIL Core repository to a directory.
...
It is not necessary to build wallet functionality to run `bitcoinild` or  `bitcoinil-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/bitcoinil/bitcoinil/blob/master/doc/descriptors.md), `sqlite` is required.
...
BitcoinIL Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/bitcoinil/bitcoinil/issues/7714).
...
You can deploy a `.dmg` containing the BitcoinIL Core application using `make deploy`.
...
## Building BitcoinIL Core

### 1. Configuration

There are many ways to configure BitcoinIL Core, here are a few common examples:
...
Run the following in your terminal to compile BitcoinIL Core:
...
## Running BitcoinIL Core

BitcoinIL Core should now be available at `./src/bitcoinild`.
If you compiled support for the GUI, it should be available at `./src/qt/bitcoinil-qt`.

The first time you run `bitcoinild` or `bitcoinil-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/BitcoinIL/
```
...
./src/qt/bitcoinil-qt -server # Starts the bitcoinil-qt server mode, allows bitcoinil-cli control
```
